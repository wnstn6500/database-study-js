...
CREATE TABLE burgers(
    ...
    -- VARCHAR(N): 문자를 최대 N자리까지 저장, 문법상 필수(가변 길이 타입이라 저장될 수 있는 최대 길이의 제한을 알아야 하기 떄문)
...
UPDATE burgers
set price = 500
where name = '빅맥'; -- 특정 대상값 변경 시, 조건은 반드시 기본키를 사용
...
-- '슈비두밤 버거' 가 단종됐다면, 이를 위한 데이터 삭제 쿼리는?
