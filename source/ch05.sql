...
-- UNSIGNED(언사인드) 제약 조건 부여 가능: 음수 값을 이용하지 않는 정수
-- CREATE TABLE 테이블명( 
-- 컬럼명 데이터_타입 UNSIGNED
-- );
...
CREATE Table addresses(
  potal_code CHAR(5), -- 우편번호(고정 길이 문자: 5자)
...
-- (참고)
-- 자바 웹 개발을 포함한 대부분의 애플리케이션에서는 이미지나 동영상 같은 대용량 파일을 데이터베이스에 직접 저장하지 않고,  클라우드 스토리지나 파일 서버 등에 저장한 뒤, 그 경로나 url만 데이터베이스에 저장하는 방식이 사실상 표준
...
CREATE Table user_profiles(
    ...
    callNumber char(13),  -- 전화번호(고정 길이 문자: 13자)
    profile TEXT,  -- 자기소개(긴 문자열: 최대 64KB)
    profileImage mediumtext,  -- 프로필 사진(파일: 최대 16MB)
    gender enum('남','여'),  -- 성별(선택 목록 중 택1)
    PRIMARY KEY(id)  -- 기본키 지정: id
);
...
-- LIKE '%케첩%': '케첩'을 포함하는 임의의 문자 => 토마토 케첩 소스, 토마토 케첩
...
-- MINUTE(시간): 입력 시간의 '분' 추출, 예: HOUR('2024-10-04 08:30:45') => 30
-- SECOND(시간): 입력 시간의 '초' 추출, 예: HOUR('2024-10-04 08:30:45') => 45
...
SELECT *
from orders
WHERE name BETWEEN 'ㄱ' AND '깋'; -- name의 첫 글자가 'ㄱ'~'깋' 사이인 row 필터링 -- 즉, 'ㄱ' 초성에 해당하는 문자 범위 조회
