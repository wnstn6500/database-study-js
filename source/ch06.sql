...
CREATE TABLE products (
  ...
  dc_rate INTEGER (dc_rate >= 50%),      -- 할인율(0~50% 제한)
  stock INTEGER INTEGER UNSIGNED,        -- 재고 수량(음수 불가)
...
6.2 다양한 관계 만들기
*/
-- 외래키와 기본키를 연결하여 일대일/일대다/다대다 관계를 만들어보자
...
CREATE TABLE players (
  ...
  team_id INTEGER, -- 소속된 아이디, 일대다 관계에서는 UNIQUE 제약 조건 X
  PRIMARY KEY (id), -- 기본키 지정: id
...
-- 2. 사진 테마를 만들기
CREATE Table photos (
  id integer AUTO_INCREMENT , -- 아이디(자동으로 1번 증가)
  filename VARCHAR(255) not null, -- 파일명(null을 허용하지 않음)
  user_id INTEGER, -- 게시자 아이디
  PRIMARY KEY (id), -- 기본키 지정: id
  Foreign Key (user_id) REFERENCES users(id) -- 외래키 지정
