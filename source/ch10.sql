-- Active: 1749607631677@@127.0.0.1@3306@data_modeling
/*
10. 데이터 모델링
10.1 데이터 모델링이란
*/
...
-- ① 엔티티 : 비즈니스에서 중요 대상을 모형화한 것으로 데이터베이스로 구현했을 때 테이블이 됨
-- ② 인스턴스 : 엔티티의 실제 데이터로, 테이블에 저장된 행 단위의 데이터
-- ③ 속성 : 엔티티의 특성이나 정보를 나타내는 항목으로 데이터베이스로 구현했을 때 칼럼이 됨
-- ④ 카디널리티 : 엔티티의 한 인스턴스가 다른 엔티티의 몇 개 인스턴스와 연결되는지를 나타내는 개념
-- ⑤ 기본 식별자 : 엔티티 내에서 각 인스턴스를 고유하게 식별하는 속성 또는 속성의 조합
...
CREATE Table products(
  id INTEGER AUTO_INCREMENT, -- 아이디 자동으로 1씩증가
...
CREATE Table order_details(
  ...
  count INTEGER not NULL CHECK(count > 0), -- 수량
  PRIMARY key(id),
  Foreign Key (order_id) REFERENCES orders(id), -- 외래키 지정
  Foreign Key (product_id) REFERENCES products(id),
  UNIQUE (order_id, product_id) -- 고유키 지정: order_id, product_id 하나의 주문에 같은 상품이 중복 담기지 않게 하기 위함
